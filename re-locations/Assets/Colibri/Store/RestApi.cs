...
namespace HCIKonstanz.Colibri.Store
{
    public class RestApi : SingletonBehaviour<RestApi>
    {
        public async Task<T> Get<T>(string appName, string objectName)
        {
            ...
            using (UnityWebRequest request = UnityWebRequest.Get("http://" + SyncConfiguration.SERVER_IP + ":8001/api/store/" + appName + "/" + objectName))
            {
        ...
        public async Task<bool> Put(string appName, string objectName, object putObject)
        {
            ...
            using (UnityWebRequest request = UnityWebRequest.Put("http://" + SyncConfiguration.SERVER_IP + ":8001/api/store/" + appName + "/" + objectName, jsonData))
            {
        ...
        {
            using (UnityWebRequest request = UnityWebRequest.Delete("http://" + SyncConfiguration.SERVER_IP + ":8001/api/store/" + appName + "/" + objectName))
            {
