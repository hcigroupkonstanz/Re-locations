...
public class SyncCommands
{
    ...
    public void AddModelUpdateListener(string channel, Action<JObject> listener)
    {
        AddListener(channel, _modelUpdateListeners, listener);
        _connection.SendCommand(channel, "modelInitialState", null);
    }

    public void AddModelUpdateListener(string channel, Action<JObject> listener, int fetchInitialStateId)
    {
        AddListener(channel, _modelUpdateListeners, listener);
        _connection.SendCommand(channel, "modelInitialState", new JObject { { "Id", fetchInitialStateId } });
    }

    public void AddModelUpdateListener(string channel, Action<JObject> listener, string fetchInitialStateId)
    {
        AddListener(channel, _modelUpdateListeners, listener);
        _connection.SendCommand(channel, "modelInitialState", new JObject { { "Id", fetchInitialStateId } });
    }

    public void RemoveModelUpdateListener(string channel, Action<JObject> listener) => RemoveListener(channel, _modelUpdateListeners, listener);

    public void AddModelDeleteListener(string channel, Action<JObject> listener) => AddListener(channel, _modelDeleteListeners, listener);
    public void RemoveModelDeleteListener(string channel, Action<JObject> listener) => RemoveListener(channel, _modelDeleteListeners, listener);
}
